...
One is about the Levi-Civita connection, and the other is a result of John Nash~\cite{nash-1956}, which states that \textit{any Riemannian manifold is isometric to a smooth submanifold of a Euclidean space of sufficiently large dimension}.
In other words, Riemannian manifolds can be defined as submanifolds of Euclidean spaces with induced metrics.

The original proof had to deal with the loss of derivatives;
it was done by a delicate and, at the time, unprecedented iteration scheme.
...
His proof uses a contraction argument, as in the standard inverse function theorem in Banach spaces.

Thanks to Günther's simplification, I was able to include this theorem in a differential geometry course.
These notes grew out of that course, and I hope they are worth sharing.
...
Section~\ref{sec:Reductions} consists of several steps that reduce the embedding problem to a perturbation problem of a metric on the torus.
In Section~\ref{sec:free}, we introduce free maps and prove Nash's lemma.
In the following section, we formulate Günther's lemma and use these lemmas to solve the perturbation problem and therefore the embedding problem.
The final section proves Günther's lemma; the rest of the paper helps one see its power.
...
The derivative of $w$ in the direction of a tangent vector $\vec x \in \T_p\Omega$ is denoted by $\vec x w$, and \label{dw}\index{4d@$w^*$, $dw$, $\vec x w$} $\vec x w = dw(\vec x)$;
...
The system \ref{eq:<ww>=g} makes perfect sense for $C^1$ maps $w$, but the proof of the bound $d\ge \tbinom{n+1}{2}$ essentially uses that $w$ is $C^k$-smooth for sufficiently large $k$.
...
%It might happen that the optimal dimension for $C^2$-embeddings is smaller than $\tbinom{n+1}{2}$. This question was mentioned by Mikhael Gromov and Vladimir Rokhlin in the cited paper. Counting degrees of freedom of the curvature tensor gives a lower bound that is still quadratic in $n$; namely, $d\ge \tfrac1{6}\cdot n\cdot (n+5)$, also check \cite{petrunin-2025}.
...
where $\ell_r\:\RR\to\SSS^1_r$ is a length-preserving covering map, say $\ell_r(x)=(r\cdot\cos\tfrac xr,r\cdot\sin\tfrac xr)$,
...
Then there exists a metric tensor $h$ on $\Omega$ and a one-parameter family of smooth maps $w_t\:\Omega \to \RR^d$ for $t>0$ with the induced metrics $g_t=g+t\cdot h$.
\end{thm}
...
Finally, choose $w_3\:\Omega\hookrightarrow \RR^{2\cdot n+1}$ provided by the Whitney embedding theorem, and observe that $(w_1 \oplus w_3)\ominus (w_2\oplus w_3)$ is the required smooth isometric embedding.
\qeds
...
\paragraph{Step 4.}
Now we sketch the reduction of the main theorem to the compact case provided in Step 3. A complete proof is given in \cite[Part~D]{nash-1956}.
...
is constant, say $c_{\mathfrak{i}}$, outside of a compact subset $K_{\mathfrak{i}}\subset \Omega$ and
...
\section{Free maps and Nash's lemma}\label{sec:free}

Here we introduce free maps and prove Nash's lemma.

From now on, we look only at the torus case.
Our torus $\TT^n\z=\RR^n/\ZZ^n$ is equipped with cyclic coordinates and globally defined partial derivatives $\partial_1,\dots,\partial_n$.
Furthermore, a metric tensor on $\TT^n$ can be (and will) described by a smooth map $g\:\TT^n\to\RR^N$ with components $g_{ij}$ for $1\le i\le j\le n$; here and further
\[N\df\tfrac{n\cdot(n+1)}2.\]\label{Nn}
...
There is a smooth free embedding $f\:\TT^n\hookrightarrow \RR^d$.
\end{thm}

From now on a free map $f\:\TT^n\hookrightarrow \RR^d$\label{f} is fixed and we may treat $\TT^n$ as a submanifold of $\RR^d$.
The tangent space $\T_p=\T_p\TT^n$ is the $n$-dimensional space spanned by the first-order partial derivatives $\partial_1f,\z\dots, \partial_nf$ at~$p$.
Let $\T_p^\perp$ denote the orthogonal complement of $\T_p$ in $\RR^d$.
Further, define the osculating space $\T^2_p$ as the span of all first- and second-order partial derivatives $\partial_i f$ and $\partial_{ij} f$ at $p$, and set $\N_p \z\df \T^2_p \cap \T_p^\perp$.
...
Recall that $f\:\TT^n \hookrightarrow\RR^d$ is a fixed smooth free embedding
...
We will see soon that the perturbation theorem follows if the quadratic equation $L(w)+Q(w,w)=h$ has a solution for any sufficiently small metric tensor $h$.
...
\section{Günther's lemma and perturbation}\label{sec:perturbation}
...
\paragraph{Perturbation.}
We now complete the proof of Nash's theorem, modulo Günther's lemmas.
...
admits a smooth solution $w\:\TT^n\to\RR^d$ for any smooth metric tensor~$h$ (not necessarily Riemannian) with $\|h\|_{C^{2,\alpha}}\z<r$.
...
It is sufficient to find a smooth map $w\:\TT^n\to\RR^d$ such that
\[
Q(f+w, f+w) = g_0 + h,
\]
...
The trick is to choose $\vec x$ so that it absorbs high-frequency terms and dissolves the loss-of-derivatives problem; see \ref{eq:Delta}, \ref{eq:AB=<C}, and \ref{eq:tildeQ} below.

In the proof of \ref{lem:perturb}, we found a correction term $w$ of the free embedding $f$ such that
...
Note that perturbing an embedding in a tangential direction does not affect the Riemannian metric to first order;
it only changes the parametrization.
For this reason, introducing a tangential term might look strange, but it works.
Similar ideas appear in the DeTurck trick for the Ricci flow \cite[2.2]{brendle} and in Kuiper's improvement of Nash's $C^1$-embedding theorem~\cite{kuiper-1955}.
...
\end{align*}
Averaging this inequality for $\tfrac1{10}$-balls in $\TT^n$ leads to the second inequality in \ref{eq:biLip}.
\qeds
