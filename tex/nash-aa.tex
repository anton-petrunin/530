...
Later, Matthias Günther discovered a magic way to remove this loss-of-derivatives problem \cite{gunter}.
...
In other words, we can take the functions $x_i$ and $x_i\pm x_j$ for all $i< j$ as our functions $\psi_1,\dots,\psi_d$ (these should be smoothly extendable to the whole manifold)
...
\begin{thm}{Problem}
Show that any metric tensor $g$ on a smooth manifold $\Omega$ is induced by a smooth embedding $\Omega\hookrightarrow \RR^r\ominus\RR^s$ for some large $r$ and $s$.
\end{thm}
...
Next proof follows the argument of the inverse function theorem in Banach spaces; see, for example,~\cite{cartan}.
...
Тhere is a sequence of constants $c_2,c_3,\dots$ such that
\[
\begin{aligned}
