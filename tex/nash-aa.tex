...
This construction led to the Nash--Moser theorem, which has found many applications.
...
Other results, including Schauder's estimates and the Whitney embedding theorem can be treated as black boxes.
...
Section~\ref{sec:Reductions} consists of several steps that reduce the embedding problem to the realization of a perturbed  metric on the torus.
In Section~\ref{sec:free}, we introduce free maps and prove Nash's lemma.
In the following section, we formulate Günther's lemma and use it to realize a perturbed  metric and therefore prove the embedding problem.
The final section proves Günther's lemma.
This is the main part of the paper; the other sections are only there to show the power of this lemma.
...
more formally, the derivative of $w$ in the direction $\vec x$ is the \index{differential}\emph{differential} $dw\colon\T\Omega \to \T\bar\Omega$ of $w$, evaluated at $\vec x$.
...
By Nash's theorem, we can find two smooth embeddings $w_1, w_2 \colon \Omega \hookrightarrow \RR^d$ whose induced metrics are $g_1$ and $g_2$, respectively.
...
\paragraph{Realization of a perturbed  metric.}
...
By \ref{prop:approx-nash}, there exists a metric tensor $h$ such that for any $t>0$, there is a smooth map $w_1\colon\TT^n\to \RR^{d_1}$ that induces $g_1+t\cdot h$.
...
Choose your favorite smooth regular embedding $w_1\colon\TT^n\hookrightarrow \RR^{n+1}$.
...
It remains to take a smooth isometric embedding $w\colon (\TT^{2\cdot n+1}, \bar g) \hookrightarrow \RR^d$ provided by Step 2 and observe that the composition $w \circ \iota\colon(\Omega, g) \hookrightarrow \RR^d$ is an isometric embedding.
...
We will see soon that realization of a perturbed metric follows if the quadratic equation
\[L(w)+Q(w,w)=h\]
...
\section{Realization of a perturbed  metric}\label{sec:perturbation}
...
In Section \ref{sec:Reductions}, we showed that realization of a perturbed  metric (\ref{thm:restricted}) implies the main theorem (\ref{thm:main}).
Now we will show that the lemma implies realization of a perturbed  metric (\ref{thm:restricted}), and therefore the main theorem (\ref{thm:main}).
...
In the proof of the lemma, we will follow the argument of the classical inverse function theorem in Banach spaces; see, for example, \cite{cartan}.
...
The trick is to choose $\vec x$ so that the inequalities in the lemma hold; see \ref{eq:Delta}, \ref{eq:AB=<C}, and \ref{eq:tildeQ} below.

The correction term $w$ in the proof of \ref{lem:perturb} meets the equation
...
\[(\Delta-1)\colon u\mapsto \Delta u-u\]
defines a bi-Lipschitz isomorphism $C^{k,\alpha}(\TT^n)\to C^{k-2,\alpha}(\TT^n)$;
moreover the Lipschitz constants can be chosen independently of $k$.

In other words, there is a constant $c>0$ such that the operator
\[(\Delta-1)\colon C^{k,\alpha}(\TT^n)\to C^{k-2,\alpha}(\TT^n)\]
is $c$-bounded and has $c$-bounded inverse $(\Delta-1)^{-1}$ for each $k\ge 2$.
\end{thm}
...
\end{align*}
Averaging this inequality for $\tfrac1{10}$-balls in $\TT^n$ leads to the second inequality in \ref{eq:biLip} for $k=2$.
Finally, since $\partial_i\Delta=\Delta\partial_i$ for every $i$, we get \ref{eq:biLip} for any $k$.
\qeds
