...
Recall that by \emph{Liouville's theorem}, the geodesic flow $\phi^t\:\S M\z\to \S M$ preserves the natural volume on $\S M$ in its domain of definition.
...
Note that $\partial_+\S  M$ is a bundle over $\partial M$ with fibers formed by closed half-spheres.
...
The full boundary $\partial\S M$ is a sphere bundle over $\partial M$;
it contains $\partial_+ \S M$ as a subbundle.
...
so $s$ is a partially defined involution on $\S M$ which we will call \emph{scattering map}.
...
If in addition their hitting time functions  coincide (that is, if $\ell(\vec u)\equiv\bar\ell(\vec u)$ for any $\vec u\in \partial_+ \S M$), then we say that $M$ and $\bar M$ have the identical \emph{lens data}.

Notice that if a manifold contains a copy of a round hemisphere, then cutting it and gluing the opposite points of its boundary produces a manifold with identical scattering data.
\begin{figure}[h!]
\centering
...
\begin{thm}{Exercise}
Show that the above construction does not work if instead of bump we use a dimple.
\end{thm}
...
Suppose that the complement $\gamma\setminus K$ of any nontrivial $g$-geodesic $\gamma$ coincides with the complement of a line (as sets).
...
In other words, if $\bar\gamma$ enters $\bar M$, then the complement $\bar\gamma\setminus \bar M$ has two connected components which are parts of a line $\gamma$ in $E$.
...
The vector fields as $\vec i$ is an example of the so-called \emph{Jacobi fields} along $\gamma_\tau$;
they describe a variation of a geodesics, and we will see them again.

\begin{thm}{Exercise}\label{ex:exit-time}
Let $M$ be a connected manifold with boundary.
Assume that hitting time in $M$ is bounded by $\ell$;
that is, any geodesic that has one end on $\partial M$ has length at most $\ell$.
Show that any geodesic in $M$ has length at most $\ell$.
\end{thm}

\parit{Hint:} Show that the set of vectors $\vec u\in\S M$ such that the arc $\gamma_{\vec u}|_{[0, \ell]}$ lies in $M$ is open and closed;
here the geodesic $\gamma_{\vec u}$ defined by $\gamma'_{\vec u}(0)=\vec u$.
...
and by $\bar\phi^t\:\S \bar M\to \S \bar M$ its the geodesic flow for time $t$.
...
\item $\ell\:\S M\to [0,\infty]$ denotes the hitting time of $\gamma_{\vec u}$ in the boundary of $M$.
...
By taking $f\equiv 1$ in the above theorem, we get the following.


\begin{thm}{Corollary}
If $M$ is as in the theorem, then
 \[\vol\SSS^{n-1}\cdot\vol M=\int_{\vec u\in \partial_+\S  M} \ell(\vec u)\cdot\langle\vec u,\vec n\rangle.\]
\end{thm}


\parit{Proof.}
Since any unit-speed geodesic in $M$ hits its boundary in finite time,
any vector $\vec w\in\S M$ can be uniquely described as $\phi^t(\vec u)$ for some $\vec u\in \partial_+\S  M$ and $0\le t\le \ell(\vec u)$.
...
Note that both forms are invariant with respect to vertical shifts (along $\RR$).
...
on $\partial \S M$.
Since both forms $\omega$ and $\alpha$ are invariant with respect to vertical shifts, we get that \ref{eq:omega/chi} holds everywhere in $\Phi$.
\qeds
...
Moreover, if a unit vector $\vec u\in\T_x$  points in the direction of the unique geodesic $\gamma$, then 
...
Choose a closed set $A$, a point $x\notin A$.
Let $\gamma$ be a unit-speed minimizing geodesic from $x$ to $A$; denote by $y\in A$ the other end of $\gamma$.

Choose a smooth unit-speed curve $\alpha$ that such that $\alpha(0)=x$, and set $\vec w=\alpha'(0)$.
Observe that 
\begin{align*}
...
\end{align*}
for any fixed $\eps>0$.
Applying the triangle inequality for $y$, $\gamma(\tfrac t\eps)$, and $\alpha(t)$, we get  that
\begin{align*}
f\circ \alpha(t)&=\dist{y}{\alpha(t)}{M}\le
\\
&\le \dist{y}{\gamma(\tfrac t\eps)}{M}+\dist{\gamma(\tfrac t\eps)}{\alpha(t)}{M}\le
\\
&\le\dist{p}{x}{}-t\cdot\langle\vec u,\vec w\rangle+O(\eps\cdot t).
\end{align*}
Passing to the limit as $\eps\to0$, we get
\[(f\circ\alpha)'(0)\le-\langle\vec u,\vec w\rangle\eqlbl{f'=<u,v>}\]
...
for any $\vec w\in\T_x$.
Since both sides of the last inequality are linear in $\vec w$, we get that the equality  
\[d_xf(\vec w)=-\langle\vec u,\vec w\rangle\]
...
Assume that for any $p\in \partial M$ the function $\distfun_p$ is differentiable on $\partial M\setminus\{p\}$.
...
\begin{subthm}{}
For any $p\in M$ the distance function $\distfun_p$ is differentiable in $M\setminus\{p\}$.
\end{subthm}
...
\begin{thm}{Area inequality}\label{cor:area-inequality}
...
Denote by $A$, $A'$, and $B$, $B'$ the opposite sides of the square~$\square$.
...
The set $s^{-1}(\RR^2\setminus \square)$ is an open subset of $\square$.
If it is nonempty, then it has positive volume.
In this case
\[\vol(\square,g)>\vol(s^{-1}(\square),g)\ge 1\]
--- a contradiction.

Summarizing: there is a geodesic path of unit $g$-length connecting any point on one side of the square to a point on the opposite side.

Moreover, for any pair of opposite facets and an interior point $p\in\square$, there is a unique geodesic path of unit $g$-length from one face to the other that passes thru $p$.
...
jointly with  Bruce Kleiner, he constructed more examples \cite{croke-kleiner-1994}.
